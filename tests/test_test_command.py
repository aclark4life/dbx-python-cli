...
def test_test_switches_to_default_branch(tmp_path):
    """Test that dbx test switches to the configured default_branch before running tests."""
    repos_dir = tmp_path / "repos"
    repos_dir.mkdir(parents=True)
...
[repo.groups.django.default_branch]
        ...
        with patch("dbx_python_cli.commands.test.get_venv_info") as mock_venv:
            with patch("subprocess.run") as mock_run:
                ...
                switch_calls = [
                    ...
                    if c.args and "switch" in c.args[0]
                ]
...
def test_test_no_branch_switch_when_not_configured(mock_config, temp_repos_dir):
    """Test that no git switch is called when default_branch is not configured."""
    with patch("dbx_python_cli.commands.repo_utils.get_config_path") as mock_get_path:
        with patch("dbx_python_cli.commands.test.get_venv_info") as mock_venv:
            with patch("subprocess.run") as mock_run:
                mock_get_path.return_value = mock_config
                ...
                # No git switch should be called
                switch_calls = [
                    c
                    for c in mock_run.call_args_list
                    if c.args and "switch" in c.args[0]
...
def test_test_branch_switch_failure_is_non_fatal(tmp_path):
    """Test that a failed branch switch emits a warning but tests still run."""
    repos_dir = tmp_path / "repos"
    repos_dir.mkdir(parents=True)
...
[repo.groups.pymongo.default_branch]
mongo-python-driver = "feature-branch"
"""
    config_path.write_text(config_content)
    ...
    def _mock_run(cmd, **kwargs):
        ...
        if "switch" in cmd:
            mock.returncode = 1
            mock.stderr = "error: pathspec 'feature-branch' did not match any file(s)"
        else:
            mock.returncode = 0
            mock.stderr = ""
        return mock
    ...
    with patch("dbx_python_cli.commands.repo_utils.get_config_path") as mock_get_path:
        with patch("dbx_python_cli.commands.test.get_venv_info") as mock_venv:
            with patch("subprocess.run", side_effect=_mock_run):
                ...
                result = runner.invoke(app, ["test", "mongo-python-driver"])
                # Tests should still run (and pass since pytest mock returns 0)
                assert result.exit_code == 0
